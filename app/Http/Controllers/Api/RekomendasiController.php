...
class RekomendasiController extends Controller
{
    ...
    public function run(Request $request)
    {
        ...
        $prodiId = $request->prodi_id ?? 0;

        $mcpClient = app(MCPClientService::class);

        try {
            $result = $mcpClient->runRekomendasiGenerate($prodiId);

            return back()->with('success', "Agent Rekomendasi selesai: {$result['recommendation_count']} rekomendasi dibuat");
        } catch (\Exception $e) {
