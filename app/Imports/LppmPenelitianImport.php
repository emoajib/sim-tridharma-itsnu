...
class LppmPenelitianImport implements ToModel, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        ...
        $judul = $row['judul_penelitian'] ?? null;
        $nidn = $row['ketua_pelaksana_nidn'] ?? $row['ketua_pelaksana_nidn'] ?? null;

        if (!$judul) {
            $this->failures[] = "Row {$this->rowNumber}: Judul penelitian kosong";
            return null;
        }
        ...
        return new Penelitian([
            ...
            'jenis_penelitian' => $row['skema'] ?? 'Penelitian Terapan',
