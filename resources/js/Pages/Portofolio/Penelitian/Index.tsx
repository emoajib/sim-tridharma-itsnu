...
export default function Index({ penelitian, dosen_list, prodi_list, periode_list, success }: Props) {
    ...
    function openEdit(item: Penelitian) {
        setEditing(item);
        setData({
    ...
    function confirmDelete(item: Penelitian) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Penelitian</h2>}
        >
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Judul Penelitian</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Jenis</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Sumber Dana</th>
                                    ...
                                    {penelitian.data.length === 0 ? (
                                        ...
                                        penelitian.data.map((item) => (
                                            <tr key={item.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-700">{item.judul_penelitian}</td>
                                                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-700">{item.jenis_penelitian}</td>
                        ...
                        {penelitian.last_page > 1 && (
                            <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4">
                                <div className="text-sm text-gray-700">
                                    Menampilkan {penelitian.from} - {penelitian.to} dari {penelitian.total}
                                </div>
                                <div className="flex gap-1">
                                    {penelitian.links.map((link, i) => (
                                        <button
                                            key={i}
                                            disabled={!link.url}
                                            onClick={() => {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="text-lg font-semibold text-gray-900">{editing ? 'Edit Penelitian' : 'Tambah Penelitian'}</h3>
                        ...
                        <form onSubmit={submit}>
                            ...
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Judul Penelitian</label>
                                <input type="text" value={data.judul_penelitian} onChange={(e) => setData('judul_penelitian', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                {errors.judul_penelitian && <p className="mt-1 text-xs text-red-600">{errors.judul_penelitian}</p>}
                            </div>
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Jenis Penelitian</label>
                                <select value={data.jenis_penelitian} onChange={(e) => setData('jenis_penelitian', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="">Pilih Jenis</option>
                                    <option value="mandiri">Mandiri</option>
                                    <option value="kelompok">Kelompok</option>
                                    <option value="hibah">Hibah</option>
                                </select>
                                {errors.jenis_penelitian && <p className="mt-1 text-xs text-red-600">{errors.jenis_penelitian}</p>}
                            </div>
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Sumber Dana</label>
                                <input type="text" value={data.sumber_dana} onChange={(e) => setData('sumber_dana', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                {errors.sumber_dana && <p className="mt-1 text-xs text-red-600">{errors.sumber_dana}</p>}
                            </div>
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Jumlah Dana</label>
                                <input type="number" value={data.jumlah_dana} onChange={(e) => setData('jumlah_dana', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                {errors.jumlah_dana && <p className="mt-1 text-xs text-red-600">{errors.jumlah_dana}</p>}
                            </div>
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Tahun Pelaksanaan</label>
                                <input type="number" value={data.tahun_pelaksanaan} onChange={(e) => setData('tahun_pelaksanaan', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                {errors.tahun_pelaksanaan && <p className="mt-1 text-xs text-red-600">{errors.tahun_pelaksanaan}</p>}
                            </div>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
                        ...
                        <p className="mb-4 text-sm text-gray-600">Yakin ingin menghapus <strong>{deleteTarget.judul_penelitian}</strong>?</p>
                        <div className="flex justify-end gap-2">
