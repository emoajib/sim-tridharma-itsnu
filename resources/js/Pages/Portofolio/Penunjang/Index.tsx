...
interface Props {
    penunjang: PaginatedData<Penunjang>;
    dosen_list: Dosen[];
    prodi_list: Prodi[];
...
export default function Index({ penunjang, dosen_list, prodi_list, periode_list, success }: Props) {
    const [search, setSearch] = useState(() => {
        return new URLSearchParams(window.location.search).get('search') || '';
    });
    const [showModal, setShowModal] = useState(false);
    const [editing, setEditing] = useState<Penunjang | null>(null);
    const [deleteTarget, setDeleteTarget] = useState<Penunjang | null>(null);
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            router.get(route('portofolio.penunjang'), { search }, { preserveState: true, replace: true });
        }, 500);
    ...
    function openEdit(item: Penunjang) {
        setEditing(item);
        setData({
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (editing) {
            put(route('portofolio.penunjang.update', editing.id), {
                onSuccess: () => { setShowModal(false); reset(); setEditing(null); },
            });
        } else {
            post(route('portofolio.penunjang.store'), {
                onSuccess: () => { setShowModal(false); reset(); },
            });
        }
    };

    function confirmDelete(item: Penunjang) {
        setDeleteTarget(item);
    }

    function executeDelete() {
        if (!deleteTarget) return;
        destroy(route('portofolio.penunjang.destroy', deleteTarget.id), {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Kegiatan Penunjang</h2>}
        >
            <Head title="Kegiatan Penunjang" />
            ...
            <div className="py-12">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        ...
                        {penunjang.last_page > 1 && (
                            <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4">
                                <div className="text-sm text-gray-700">
                                    Menampilkan {penunjang.from} - {penunjang.to} dari {penunjang.total}
                                </div>
                                <div className="flex gap-1">
                                    {penunjang.links.map((link, i) => (
                                        <button
                                            key={i}
                                            disabled={!link.url}
                                            onClick={() => {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="text-lg font-semibold text-gray-900">{editing ? 'Edit Kegiatan Penunjang' : 'Tambah Kegiatan Penunjang'}</h3>
