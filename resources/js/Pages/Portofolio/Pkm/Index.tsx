...
export default function Index({ pkm, dosen_list, prodi_list, periode_list, success }: Props) {
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Pengabdian Kepada Masyarakat</h2>}
        >
            <Head title="PKM" />
                                    ...
                                    {pkm.data.length === 0 ? (
                                        ...
                                        pkm.data.map((item) => (
                                            <tr key={item.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-700">{item.judul_pkm}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="text-lg font-semibold text-gray-900">{editing ? 'Edit PKM' : 'Tambah PKM'}</h3>
                        ...
                        <form onSubmit={submit}>
                            ...
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Judul PKM</label>
                                <input type="text" value={data.judul_pkm} onChange={(e) => setData('judul_pkm', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                {errors.judul_pkm && <p className="mt-1 text-xs text-red-600">{errors.judul_pkm}</p>}
                            </div>
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Jenis PKM</label>
                                <input type="text" value={data.jenis_pkm} onChange={(e) => setData('jenis_pkm', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                {errors.jenis_pkm && <p className="mt-1 text-xs text-red-600">{errors.jenis_pkm}</p>}
                            </div>
                            <div className="mb-4">
                                <label className="mb-1 block text-sm font-medium text-gray-700">Lokasi</label>
                                <input type="text" value={data.lokasi} onChange={(e) => setData('lokasi', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
                        ...
                        <p className="mb-4 text-sm text-gray-600">Yakin ingin menghapus <strong>{deleteTarget.judul_pkm}</strong>?</p>
                        <div className="flex justify-end gap-2">
