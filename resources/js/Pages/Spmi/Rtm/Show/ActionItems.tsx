...
import { Plus, CheckCircle2, XCircle, Edit3, Trash2, PlayCircle } from 'lucide-react';
...
function countdownLabel(deadline: string | null): { text: string; isOverdue: boolean; isUrgent: boolean } | null {
    ...
    if (diffDays <= 7) return { text: `${diffDays} hari lagi`, isOverdue: false, isUrgent: true };
...
export default function ActionItems({ rtmId, actionItems, user_list, onDeleteAction }: Props) {
    ...
    function handleAddAction() {
        ...
        router.post(
            route('spmi.rtm.action-item.store', rtmId),
            { deskripsi: actionDeskripsi, pic_user_id: actionPicId, deadline: actionDeadline },
            {
                preserveScroll: true,
                onSuccess: () => {
                    setActionDeskripsi(''); setActionPicId(''); setActionDeadline('');
                    setShowActionForm(false); setActionProcessing(false);
                },
    ...
    function handleUpdateAction() {
        ...
        router.put(
            ...
            { deskripsi: editActionDeskripsi, pic_user_id: editActionPicId, deadline: editActionDeadline },
            {
                preserveScroll: true,
                onSuccess: () => {
                    setEditActionId(null); setEditActionDeskripsi('');
                    setEditActionPicId(''); setEditActionDeadline('');
    ...
    function transitionAction(item: ActionItem, status: string) {
        router.post(route('spmi.rtm.action-item.transition', item.id), { status }, { preserveScroll: true });
    }
    ...
    function handleSaveResult() {
        ...
        router.post(
            ...
            {
                preserveScroll: true,
                onSuccess: () => { setResultInput(null); setActionProcessing(false); },
                onError: () => setActionProcessing(false),
            }
        );
    }
    ...
    return (
        <div className="p-6">
                ...
                <div ref={formRef} className="mb-6 rounded-lg border border-indigo-200 bg-indigo-50 p-4">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="mb-1 block text-xs font-medium text-gray-700">PIC</label>
                                <select
                                    value={actionPicId}
                        ...
                        <div className="flex items-center justify-end gap-2">
                            <button
                                onClick={() => { setShowActionForm(false); setActionDeskripsi(''); setActionPicId(''); setActionDeadline(''); }}
            ...
            {actionItems.length === 0 ? (
                <p className="py-8 text-center text-sm text-gray-400">Belum ada action item.</p>
            ) : (
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">PIC</th>
                                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Deadline</th>
                                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Hasil</th>
                                ...
                                return (
                                    <tr key={item.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="whitespace-nowrap px-4 py-4 text-sm">
                                            {isEditing ? (
                                                ...
                                                <>
                                                    ...
                                                    {countdown && (
                                                        <span className={`ml-1 text-[10px] ${countdown.isOverdue ? 'text-red-600 font-semibold' : countdown.isUrgent ? 'text-orange-600 font-semibold' : 'text-gray-400'}`}>
                                                            ({countdown.text})
                                                        </span>
                                                    )}
                                                </>
                                            ...
                                            {resultInput?.id === item.id ? (
                                                <div className="flex items-center gap-1">
                                                    <input
                                                        ...
                                                        onChange={(e) => setResultInput({ ...resultInput, hasil: e.target.value })}
                                                        className="flex-1 rounded-lg border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                                        placeholder="Hasil..."
                                                    />
                                                    <button onClick={handleSaveResult} disabled={!resultInput.hasil.trim() || actionProcessing} className="rounded bg-green-600 px-2 py-1 text-[10px] text-white hover:bg-green-700 disabled:opacity-50">Simpan</button>
                                                    <button onClick={() => setResultInput(null)} className="rounded bg-gray-200 px-2 py-1 text-[10px] text-gray-600 hover:bg-gray-300">Batal</button>
                                                </div>
                                            ) : (
                                                item.hasil || '-'
                                            )}
                                        ...
                                        <td className="whitespace-nowrap px-4 py-4">
                                            <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${actionStatusBadge[item.status] || 'bg-gray-100 text-gray-800'}`}>
                                                {item.status === 'open' ? 'Open' : item.status === 'in_progress' ? 'In Progress' : item.status === 'completed' ? 'Completed' : item.status === 'cancelled' ? 'Cancelled' : item.status}
                                            </span>
                                        </td>
                                        <td className="whitespace-nowrap px-4 py-4 text-sm">
                                            {isEditing ? (
                                                <div className="flex items-center gap-1">
                                                    <button onClick={handleUpdateAction} disabled={!editActionDeskripsi.trim() || actionProcessing} className="rounded bg-indigo-600 px-2 py-1 text-[10px] text-white hover:bg-indigo-700 disabled:opacity-50">Simpan</button>
                                                    <button onClick={() => setEditActionId(null)} className="rounded bg-gray-200 px-2 py-1 text-[10px] text-gray-600 hover:bg-gray-300">Batal</button>
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-1">
                                                    {item.status === 'open' && (
                                                        <button onClick={() => transitionAction(item, 'in_progress')} className="rounded p-1 text-yellow-600 hover:bg-yellow-50" title="Mulai">
                                                            <PlayCircle className="h-4 w-4" />
                                                        </button>
                                                    )}
                                                    {isEditable && (
                                                        <>
                                                            <button onClick={() => startEditAction(item)} className="rounded p-1 text-gray-500 hover:bg-indigo-50 hover:text-indigo-600" title="Edit">
                                                                <Edit3 className="h-4 w-4" />
                                                            </button>
                                                            {item.status === 'in_progress' && (
                                                                <button onClick={() => setResultInput({ id: item.id, hasil: item.hasil || '' })} className="rounded p-1 text-green-600 hover:bg-green-50" title="Selesaikan">
                                                    ...
                                                    {item.status === 'open' && (
                                                        <button onClick={() => onDeleteAction(item.id)} className="rounded p-1 text-red-500 hover:bg-red-50" title="Hapus">
                                                            <Trash2 className="h-4 w-4" />
                                                        </button>
                                                    )}
                                                    {item.status === 'in_progress' && (
                                                        <button onClick={() => transitionAction(item, 'cancelled')} className="rounded p-1 text-gray-500 hover:bg-gray-100" title="Batalkan">
